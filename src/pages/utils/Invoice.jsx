...
const Invoice = ({ data }) => {
  ...
  const fnGetSubtotal = () => {
    ...
    data?.product_details?.map((item) => {
      ...
      return (subtotal += item.quantity * parseInt(itemPrice));
    });
    // let totalBeforeTAX = subtotal - row.discount;
    // let totalAfterTAX = totalBeforeTAX + (totalBeforeTAX * 10) / 100;
    return subtotal;
    // setProductTotalPrice(subtotal);
  };
  // useEffect(() => {
  //   fnGetPrices();
  // }, []);
              ...
              >
                ...
                <Grid item xs={6}>
                  <p
                    ...
                    style={{ textAlign: "right" }}
                  >
                    {fnGetSubtotal().toFixed(2) - data?.discount.toFixed(2)}
                  </p>
                </Grid>
